...
fn main() {
    ...
    let mut json_start_idx = 0;
    let len = contents.as_bytes().len();
    loop {
        let next_json_string_idx = find_next_json_string_idx(&contents, json_start_idx);
        let mut json_string = &contents[json_start_idx..next_json_string_idx-1];
        let json_object: Value = serde_json::from_str(json_string).unwrap();
        ...
        json_start_idx = next_json_string_idx;
        if (next_json_string_idx - 1) == len {
            break;
        }
    }
